...
class ActionRunner:
	...
	func iteration(next: IDialogAction)->void:
		if not next: next = DialogSystem.get_default_next(current)
		if not next:
			print("Finished, at node {0}".format([current.name]))
			if on_everything_finished: on_everything_finished.call()
			return
		current = next
...
static func get_default_next(a: Node)->IDialogAction:
	...
	while parent:
		...
		for current_idx in Vector2i(start_idx, parent.get_child_count(include_internal)):
			...
			if next: return next
		if parent is DialogSystem: break
		a = parent
